...
def get_stop_word_rankings(train_examples):
    stop_words = []
    stop_words_ranking = {}
...
def train_and_predict():
    (stop_words, stop_words_ranking) = get_stop_word_rankings(train_examples)
...
def n_fold_cross_validation():
    ...
    for curr_train, curr_test in kfold_split:
        ...
        (stop_words, stop_words_ranking) = get_stop_word_rankings(train_examples[curr_train])

        train_examples_vectorized = []
        test_examples_vectorized = []

        # Vectorizing train and test data
        for train_example in train_examples[curr_train]:
            words = get_words_from_example(train_example)
            words_list = []
            for (word, cnt) in words.items():
                words_list.append((cnt, word))
        ...
        for test_example in train_examples[curr_test]:
            words = get_words_from_example(test_example)
            words_list = []
            for (word, cnt) in words.items():
                words_list.append((cnt, word))
        ...
        model = get_model()
        model.fit(train_examples_vectorized, train_labels[curr_train])
        predicted_labels = model.predict(test_examples_vectorized)
        accuracy = accuracy_score(train_labels[curr_test], predicted_labels)
        print(f'Step: {step} Accuracy score for validation is {accuracy_score(train_labels[curr_test], predicted_labels)}')
        print(confusion_matrix(train_labels[curr_test], predicted_labels))
